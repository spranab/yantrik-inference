...
class Service:
    """Everything the handlers need. The two contexts are independent, so they
    get their own locks: a decide call can answer while a chat is streaming."""
    ...
    def __init__(self, model_path: str, limits: Limits, *, n_batch: int = 2048,
        ...
        self.llm, self.C = load_model(
            model_path, n_ctx=limits.decide_ctx, n_batch=n_batch,
        ...
        self.reader = FieldReader(self.llm, self.C, head, tail,
                                  limits.decide_seq, limits.per_seq)
        self.chat = ChatEngine(self.llm, self.C, limits.chat_ctx, n_batch,
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path == "/health":
            return self._send(200, json.dumps(dict(ok=True, model=s.name)))
    ...
    def _decide(self, req, as_json: bool):
        ...
        try:
            with s.decide_lock:
                t0 = time.time()
                if not as_json:
                    answers = s.reader.read(record, fields)
                    dt = time.time() - t0
                    return self._send(200, json.dumps(dict(
                ...
                prompt_len = len(s.reader.tok(s.reader.head + record, bos=True)) + 40 * len(fields)
                ...
                vals, _ = s.reader.read_as_json(record, fields, max_new=min(room, 900))
                dt = time.time() - t0
                return self._send(200, json.dumps(dict(
    ...
    def _chat(self, req):
        ...
        try:
            if not req.get("stream"):
                with s.chat_lock:
                    text = "".join(s.chat.stream(msgs, **kw))
                return self._send(200, json.dumps(dict(
            ...
            with s.chat_lock:
                for piece in s.chat.stream(msgs, **kw):
...
def serve(service: Service, host: str, port: int):
    ...
    print(f"  decide  {service.limits.decide_seq} sequences x {service.limits.per_seq} tokens")
    print(f"  chat    {service.limits.chat_ctx} tokens")
